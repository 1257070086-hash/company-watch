...
if(usingSnapshot){manageButton.hidden=true;document.getElementById('btn-refresh').hidden=true;document.querySelector('.status-pill').title='云端数据每天 08:12–23:52，每 20 分钟自动更新';}
...
async function openSources(){sourceShell();if(!management.open)management.showModal();sourceNotice('读取监测账号…');try{await loadRegistry();renderSourceList();sourceNotice('本地 RSS 每 12 小时串行检查，每个源读取最新 10 篇；无新文章不等于链接异常。');}catch{sourceNotice('无法连接本地来源管理服务。请确认采集器正在运行，再重新打开。',true);}}
